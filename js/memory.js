...
(function () {
  ...
  function runMatchingPairs(container, opts, onComplete) {
    ...
    deck.forEach((card) => {
      ...
      btn.textContent = '?';
      btn.dataset.symbol = '?'; // mirrored for the no-aspect-ratio CSS fallback
    ...
    function onFlip(btn, card) {
      ...
      if (first.card.symbol === second.card.symbol) {
        ...
        errors += 1;
        setTimeout(() => {
          first.btn.textContent = '?'; first.btn.dataset.symbol = '?'; first.btn.classList.remove('flipped');
          second.btn.textContent = '?'; second.btn.dataset.symbol = '?'; second.btn.classList.remove('flipped');
          first = null; second = null; lock = false;
        }, 700);
      }
    }
